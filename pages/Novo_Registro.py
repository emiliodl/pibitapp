...
def connect_to_mongo():
    uri = os.environ.get('MONGO_URI', "mongodb+srv://emiliods79:uD5A2J4o38dpk0hX@cluster0.ufpae.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
...
with tab_animal:
    ...
    with st.form("form_registro_animal"):
        ...
        animal_id_input = st.text_input("ID do Animal (único)", placeholder="Ex: ANM001")
        animal_nome_comum = st.text_input("Nome Comum")
        animal_nome_cientifico = st.text_input("Nome Científico")
        animal_sexo = st.selectbox("Sexo", ["", "Macho", "Fêmea", "Desconhecido"])
        animal_peso = st.number_input("Peso (kg)", min_value=0.0, format="%.2f", step=0.01)
        animal_hvu = st.text_input("ID do HVU (Hospital Veterinário Universitário)")
        ...
        animal_data_nascimento = st.date_input("Data de Nascimento (opcional)", value=None)
        ...
        if submit_animal:
            if not animal_id_input:
                ...
                animal_data = {k: v for k, v in animal_data.items() if v not in (None, "", [])}
...
with tab_amostra:
    ...
    with st.form("form_registro_amostra"):
        ...
        amostra_id_input = st.text_input("ID da Amostra (único)", placeholder="Ex: AMS001")
        ...
        animais_existentes = list(animais_collection.find({}, {"_id": 1, "nome_comum": 1}))
        
        animal_options = [""] + [f"{animal.get('nome_comum', 'Animal sem nome')} (ID: {animal['_id']})" for animal in animais_existentes]
        selected_animal_str = st.selectbox("Vincular a qual Animal?", options=animal_options)
        ...
        amostra_condicao = st.selectbox(
            "Condição da Amostra",
            ["", "Temperatura ambiente", "Refrigerada", "Congelada", "Sem identificação", "Coagulada", "Hemolisada"]
        )
        amostra_destino = st.text_input("Destino da Amostra (e.g., Laboratório X, Biobanco)")
        amostra_latitude = st.text_input("Latitude (opcional)")
        amostra_longitude = st.text_input("Longitude (opcional)")
        amostra_resultado_exame = st.text_input("Resultado do Exame (inicial, opcional)")
        ...
        if submit_amostra:
            if not amostra_id_input:
                ...
                st.error("Por favor, selecione um animal para vincular esta amostra.")
            else:
                amostra_data = {
                ...
                amostra_data = {k: v for k, v in amostra_data.items() if v not in (None, "", [])}
...
with tab_exame:
    ...
    with st.form("form_registro_exame"):
        ...
        exame_id_input = st.text_input("ID do Exame (único)", placeholder="Ex: EXM001")
        ...
        amostras_existentes = list(amostras_collection.find({}, {"_id": 1, "Tipo de amostra": 1}))
        
        amostra_options = [""] + [f"{amostra.get('Tipo de amostra', 'Amostra sem tipo')} (ID: {amostra['_id']})" for amostra in amostras_existentes]
        selected_amostra_str = st.selectbox("Vincular a qual Amostra?", options=amostra_options)
        ...
        exame_data_realizacao = st.date_input("Data de Realização", value=date.today())
        ...
        if submit_exame:
            if not exame_id_input:
                ...
                st.error("Por favor, selecione uma amostra para vincular este exame.")
            else:
                exame_data = {
                    "_id": exame_id_input,
                ...
                exame_data = {k: v for k, v in exame_data.items() if v not in (None, "", [])}
...
with tab_reagente:
    ...
    with st.form("form_registro_reagente"):
        ...
        reagente_id_input = st.text_input("ID do Reagente (único)", placeholder="Ex: REG001")
        reagente_nome = st.text_input("Nome do Reagente")
        reagente_fabricante = st.text_input("Fabricante")
        reagente_lote = st.text_input("Número do Lote")
        reagente_validade = st.date_input("Data de Validade", value=date.today())
        reagente_quantidade = st.number_input("Quantidade (e.g., ml, g, unidades)", min_value=0.0, format="%.2f", step=0.01)
        reagente_unidade = st.text_input("Unidade de Medida (e.g., ml, g, unidades)")
        reagente_local_armazenamento = st.text_input("Local de Armazenamento")
        reagente_observacoes = st.text_area("Observações do Reagente (opcional)")
        ...
        if submit_reagente:
            if not reagente_id_input:
                ...
                reagente_data = {
                    ...
                    "nome_reagente": reagente_nome,
                    "fabricante": reagente_fabricante,
                    "numero_lote": reagente_lote,
                    "data_validade": str(reagente_validade),
                    "quantidade": reagente_quantidade,
                    "unidade": reagente_unidade,
                    "local_armazenamento": reagente_local_armazenamento,
                    "observacoes_reagente": reagente_observacoes
                }
                reagente_data = {k: v for k, v in reagente_data.items() if v not in (None, "", [])}
